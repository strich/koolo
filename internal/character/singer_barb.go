...
func (s *SingerBarb) IsKillingCouncil() bool {
...
func (s *SingerBarb) KillMonsterSequence(
	...
	skipOnImmunities = []stat.Resist{"physical"}
	...
	for attackAttempts := 0; attackAttempts < maxAttackAttempts; attackAttempts++ {
		...
		warcryOpts := []step.AttackOption{
			step.RangedDistance(WarcryMinDistance, WarcryMaxDistance),
		}

		if err := step.SecondaryAttack(skill.WarCry, monster.UnitID, 3, warcryOpts...); err == nil {
...
func (s *SingerBarb) SwapToSlot(slot int) {
	ctx := context.Get()
	if !ctx.CharacterCfg.Character.SingerBarb.FindItemSwitch {
...
func (s *SingerBarb) killMonster(npc npc.ID, t data.MonsterType) error {
...
func (s *SingerBarb) killAllCouncilMembers() error {
...
func (s *SingerBarb) anyCouncilMemberAlive() bool {
